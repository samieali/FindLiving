...
app.get("/coordinates", function (req, res) {
  ...
  let coords =
    ...
    ad +
    "&key=AIzaSyDdB4zEdQh5Ma5zNy_SX1_QzYNliMee_tk";
  axios.get(coords).then(function (response) {
    console.log(
      "" + JSON.stringify(response.data.results[0].geometry.location, null, 4)
    );
...
function nearMe(req, res) {
  ...
  Promise.all(req.results.map((property) => property.request))
    .then(function (data) {
      data.forEach((resp, i) => {
        let property = req.results[i];
        property["api_data"] = resp.data.rows[0].elements[0];
        delete property["request"];
        if (property["api_data"].distance.value < req.maxDistance) {
          closeListings.push(property);
        }
        console.log(
          "DISTANCE: " +
            property["api_data"].distance.value +
            " < " +
            req.maxDistance
        );
      });
      ...
      closeListings.sort(
        (a, b) => a.api_data.distance.value - b.api_data.distance.value
      );
